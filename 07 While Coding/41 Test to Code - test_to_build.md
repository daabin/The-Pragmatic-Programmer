...
<a id="d24e21247"></a>
But there's still a problem. When we ask developers why they write
...
<a id="d24e21252"></a>
So what do we think is important about testing? And how do we think
...
find two likely fields in a table of who-watched-what: opened\_video
and completed\_video. To write our test data, we need to know which
...
before you start coding, you may well find the patterns in the logic
...
3. Run all tests. Verify that the only failure is the
   one you just wrote.
4. Write the smallest amount of code needed to get the test to pass, and
   verify that the tests now run cleanly.
...
- They spend inordinate amounts of time ensuring that they always have
  100% test coverage.
- They have lots of redundant tests. For example, before writing a class
  for the first time, many TDD adherents will first write a failing test
  that simply references the class's name. It fails, then they write an
  empty class definition and it passes. But now you have a test that
  ...
  to change if the class name changes later. And this is just a trivial example.
- Their designs tend to start at the bottom and work their way up. (See
  [*Bottom-Up vs. Top-Down vs. The Way You Should Do It*](#sb-butd).)
...
[*Ruthless and Continuous Testing*](<../09 Pragmatic Projects/51 Pragmatic Starter Kit - starter_kit.md#testing>).
...
(see Topic 23, [*Design by Contract*](<../04 Pragmatic Paranoia/23 Design by Contract - dbc.md#dbc>)).
...
- Pass in values between zero and the maximum expressible
  ...
  result and the original argument is less than some small fraction
  of the argument (epsilon).
...
use of one of those subcomponents. This technique is a great way to
...
<a id="d24e21789"></a>
Not to be confused with “odd hack,” ad-hoc testing is when we run poke
...
circuit board or chip, we don't have test pins in software, but we
can provide various views into the internal state of a module, without
...
<a id="d24e21977"></a>
A culture of testing means all the tests pass all the time. Ignore a spew of tests that “always fail” makes it easier to ignore all the tests, and the vicious spiral begins (see Topic 3, [*Software Entropy*](<../01 Pragmatic Philosophy/03 Software Entropy - no_broken_windows.md#no_broken_windows>)).

<a id="d24e21994"></a>
Treat test code with the same care as any production code. Keep it decoupled, clean, and robust. Don't rely on unreliable things (see Topic 38, [*Programming by Coincidence*](<38 Programming by Coincidence - coincidence.md#coincidence>)) like the absolute position of widgets in a GUI system, or exact timestamps in a server log, or the exact wording of error messages. Testing for these sorts of things will result in fragile tests.
